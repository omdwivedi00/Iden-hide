...
class S3ImageProcessor:
    ...
    def __init__(
        ...
        self.detector = detector or UnifiedDetector(settings=self.settings)
        self.visualizer = visualizer or DetectionVisualizer()

        logger.info("S3 Image Processor initialized")
    ...
    def _download_image_from_s3(self, s3_path: str) -> tuple:
        ...
        try:
            ...
            logger.info("Downloaded %s -> %s", s3_path, temp_path)
    ...
    def _upload_image_to_s3(self, local_path: str, s3_path: str) -> str:
        ...
        try:
            ...
            bucket_name, key = self._parse_s3_path(s3_path)
            logger.info("Uploading %s (%s bytes) -> s3://%s/%s", local_path, file_size, bucket_name, key)
            ...
            try:
                ...
                logger.info("Upload verified: s3://%s/%s", bucket_name, key)
            except ClientError as e:
                logger.warning("Upload verification failed: %s", e)
            
            s3_url = f"https://{bucket_name}.s3.amazonaws.com/{key}"
            logger.info("Uploaded %s -> %s", local_path, s3_url)
            return s3_url
    ...
    def _list_images_in_s3_folder(self, s3_folder_path: str) -> List[str]:
        ...
        try:
            ...
            logger.info("Listing S3 folder: s3://%s/%s", bucket_name, prefix)
            ...
            logger.info("S3 response: %s objects found", response.get('KeyCount', 0))
            ...
            if 'Contents' in response:
                for obj in response['Contents']:
                    ...
                    logger.info("Found file: %s (%s bytes, ext: %s)", key, file_size, file_ext)
                    
                    if file_ext in image_extensions:
                        s3_path = f"s3://{bucket_name}/{key}"
                        image_paths.append(s3_path)
                        logger.info("Added image: %s", s3_path)
                    else:
                        logger.info("Skipped non-image: %s", key)
            else:
                logger.warning("No objects found in s3://%s/%s", bucket_name, prefix)
            
            logger.info("Found %s images in %s", len(image_paths), s3_folder_path)
            return image_paths
    ...
    def process_s3_folder(self, 
        ...
        try:
            ...
            for i, input_s3_path in enumerate(input_images):
                logger.info("Processing %s/%s: %s", i + 1, len(input_images), input_s3_path)
                ...
                if result['success']:
                    successful_count += 1
                    logger.info("Success: %s", result['original_filename'])
                else:
